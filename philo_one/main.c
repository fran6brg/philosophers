...
**
** Pour rappel dans l'set_philos() on a lock le mutex mutex_eat_counter
...
** Si c'est le cas, on unlock stop_banquet qui fait que le main se termine
** car autrement, ca ralentirai largement les actions
**
** On usleep(1000) est INDISPENSABLE
** pour pas toujours avoir le philo->eating de locké
...
/*
** On attend à pthread_mutex_lock(&banquet.stop_banquet) jusqu'à ce qu'un
** philosophe meurt
** On va wait la ligne pthread_mutex_lock(&banquet.stop_banquet);
